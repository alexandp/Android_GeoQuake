...
public class GeoQuakeViewThread extends Thread{
	...
	private int axis_;
	private GeoQuakeDbAdapter db_;
	private SurfaceHolder holder_;
	private Context ctx_;
	private int period_;
	
	public GeoQuakeViewThread(Context ctx, SurfaceHolder holder, boolean filter,
            int axis, int period) {
		scale_paint_.setARGB(255, 255, 255, 255);
		scale_paint_.setAntiAlias(true);
		line_paint_.setARGB(255, 214, 4, 4);
	...
	float text_size = canvas_width_ / 30f;
	...
	canvas.drawText(Integer.toString(i) + "g", x,
		...
		if (history_.size() > 0) {
			...
			for (int s = (int) Math.floor(end_time);s >= Math.max(Math.floor(start_time), 0);--s) {
